...
* USING THE DAG APPROACH IN CONJUNCTION WITH SUPPORT VECTOR MACHINES         *;
*                                                                            *;
* STEPS:                                                                     *;
* CREATE A MACRO FOR TRAINING                                                *;
*     MODIFY INPUT DATA TO HAVE A TARGET COLUMN FOR EACH LEVEL OF THE TARGET *;
*     RUN AN SVM MODEL FOR EACH COMBINATION OF TARGET LEVELS AND SAVE CODE   *;
*     CREATE A TABLE THAT CONTAINS THE NAME OF THE SAVED SCORING CODE FILES  *;
* CREATE A MACRO FOR SCORING                                                 *;
*     RUN THE SCORE CODE AS BASED UPON THE DIRECTED ACYCLCIC GRAPH (DAG)     *;
*       AS DESCRIBED IN THE REFERENCE                                        *;
*     THE PREDICTED TARGET IS BASED UPON THE RESULTS OF THE DAG              *;
...
*                                                                            *;
* FOR FURTHER INFORMATION ON THE DAG AND OTHER MULTICLASS SVM APPROACHES:    *;
...
*                                                                            *;
* DAG IMPLEMENTATION IS BASED UPON: J.C. PLATT, N. CRISTIANINI, AND J.       *;
* SHAWE-TAYLOR. "LARGE MARGIN DAGS FOR MULTICLASS CLASSIFICATION." ADVANCES  *;
...
%put DATA PRE-PROCESSING TIME:  %sysfunc(putn(%sysevalf(%sysfunc(TIME())-&datetime_start.),mmss.)) (mm:ss) ;
...
%do i=1 %to &n;
    %do j=%eval(&i+1) %to &n;
        ...
        proc hpsvm data = l.ModifiedInput(where=(&&Target&i=1 OR &&Target&j=1)) tolerance = &Tolerance c = &C maxiter = &Maxiter nomiss;
            target &&Target&i;
...
%put END TIME: %sysfunc(datetime(),datetime14.);
%put ONE-VS-ONE TRAINING TIME:  %sysfunc(putn(%sysevalf(%sysfunc(TIME())-&datetime_start.),mmss.)) (mm:ss) ;
...
%put END TIME: %sysfunc(datetime(),datetime14.);
%put DAG SCORING TIME:  %sysfunc(putn(%sysevalf(%sysfunc(TIME())-&datetime_start.),mmss.)) (mm:ss) ;
...
* UTILITY MACROS                                                             *;
******************************************************************************;
*** MACRO TO MAKE THE SCORED OUTPUT FOR ONE_VS_MANY;
%macro MakeScoredDAG();
data ScoredOutput1_&n;
    set &ScoreData;
    _temp_IDvar_ensure_not_existing_ = _N_; *this var name is 32 char long to help avoid conflicts;
run;
...
proc sort data=ScoredOutput out=l.ScoredOutput(drop=_temp_IDvar_ensure_not_existing_);
...
*** SET THE INPUT AND SCORE DATA SETS;
