...
* USING THE ONE-VS-ONE APPROACH IN CONJUNCTION WITH SUPPORT VECTOR MACHINES  *;
*                                                                            *;
* STEPS:                                                                     *;
...
*     ASSIGN THE TARGET BASED UPON THE MOST VOTED FOR LEVEL                  *;
...
*                                                                            *;
* FOR FURTHER INFORMATION ON ONE-VS-ONE AND OTHER MULTICLASS SVM APPROACHES: *;
...
%put END TIME: %sysfunc(datetime(),datetime14.);
%put ONE-VS-ONE SCORING TIME:  %sysfunc(putn(%sysevalf(%sysfunc(TIME())-&datetime_start.),mmss.)) (mm:ss) ;
...
%do i=1 %to &n;
    %do j=%eval(&i+1) %to &n;
        data l.ScoredOutput;
            ...
            drop P_&Target&&level&i..1 P_&Target&&level&i..0 I_&Target&&level&i _P_;
        run;
    %end;
%end;
data l.ScoredOutput;
